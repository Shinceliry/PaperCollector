    ...
    for (const thread of threads) {
        ...
        for (const message of messages) {
            ...
            for (const paper of paperInfos) {
                ...
                const openAiResult = judgeAndSummarize(OPENAI_API_KEY, abstractText);
                const firstWord = openAiResult.trim().split(/\s+/, 1)[0].toLowerCase();
        
                if (firstWord === "no") {
                    Logger.log(`CS分野ではないと判定: ${paper.title}`);
                    continue;
                } else if (firstWord === "yes") {
                    const summary = openAiResult.trim().replace(/^yes/i, "").trim();
                    postToDiscord(DISCORD_WEBHOOK, { content: createDiscordMessage(paper, summary) });
                } else {
                    Logger.log(`Yes/No判定に失敗: ${paper.title}`);
                }
            }
        thread.moveToTrash();
        }
...
function judgeAndSummarize(apiKey, abstractText) {
    const prompt = `
    以下はある論文のアブストラクトです。まずコンピュータサイエンス分野かどうかを判定してください。
    もしCS分野でなければ、回答は "No" だけにしてください。
    もしCS分野ならば、回答の先頭に "Yes" と書き、その後にアブストラクトの日本語要約(300字以内)を記述してください。
    ---
    ${abstractText}
    ---
    `;
